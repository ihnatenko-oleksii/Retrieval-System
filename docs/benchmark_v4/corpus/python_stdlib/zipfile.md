...
Official source: https://docs.python.org/3/library/zipfile.html
...
metadata_encoding is an instance-wide setting for the ZipFile. It is not possible to set this on a per-member basis.
...
Never extract archives from untrusted sources without prior inspection. It is possible that files are created outside of path, for example, members that have absolute filenames or filenames with “..” components. This module attempts to prevent that. See extract() note.
...
Changed in version 3.14: Now respects the SOURCE_DATE_EPOCH environment variable. If set, it uses this value as the modification timestamp for the file written into the ZIP archive, instead of using the current time.
