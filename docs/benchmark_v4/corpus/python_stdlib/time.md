...
Official source: https://docs.python.org/3/library/time.html
...
adjustable: True if the clock can be set to jump forward or backward in time, False otherwise. Does not refer to gradual NTP rate adjustments.
...
Changed in version 3.5: The function now sleeps at least seconds even if the sleep is interrupted by a signal, except if the signal handler raises an exception (see PEP 475 for the rationale).
...
Here is an example, a format for dates compatible with that specified in the RFC 5322 Internet email standard. [1]
