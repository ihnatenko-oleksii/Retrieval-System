...
Official source: https://docs.python.org/3/library/re.html
...
Will try to match with yes-pattern if the group with given id or name exists, and with no-pattern if it doesn’t. no-pattern is optional and can be omitted. For example, (<)?(\w+@\w+(?:\.\w+)+)(?(1)>|$) is a poor email matching pattern, which matches '<user@host.com>' as well as 'user@host.com', but does not match '<user@host.com' nor 'user@host.com>' in their entirety (re.search() finds only 'user@host.com' in the former).
