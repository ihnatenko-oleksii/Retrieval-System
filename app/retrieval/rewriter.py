...
class QueryRewriter:
    ...
    def rewrite_query(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None) -> str:
        """
        ...
        """
        if not settings.query_rewriting_on and not chat_history:
            return query
            
        if chat_history:
            # Format chat history
            history_text = ""
            for msg in chat_history[-4:]: # Only take last 4 messages to avoid blowing up context
...
Standalone query:"""
    ...
    def expand_query(self, query: str, num_expansions: int = 2) -> List[str]:
        ...
        """
        if not settings.query_expansion_on:
            return [query]
